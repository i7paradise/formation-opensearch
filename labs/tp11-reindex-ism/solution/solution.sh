...
curl -s -X PUT "$BASE_URL/products-v2" -H 'Content-Type: application/json' \
  -d '{"settings":{"number_of_shards":3,"number_of_replicas":1},"mappings":{"properties":{"name":{"type":"text","fields":{"keyword":{"type":"keyword"}}},"description":{"type":"text","analyzer":"french"},"category":{"type":"keyword"},"price":{"type":"float"},"in_stock":{"type":"boolean"},"rating":{"type":"float"},"created_at":{"type":"date"},"tags":{"type":"keyword"}}}}' | python3 -m json.tool
...
curl -s -X POST "$BASE_URL/_reindex?wait_for_completion=true" -H 'Content-Type: application/json' \
  ...
  | python3 -c "import json,sys; r=json.load(sys.stdin); print('Copied:', r.get('created',0)+r.get('updated',0), 'Failures:', len(r.get('failures',[])))"
