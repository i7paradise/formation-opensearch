...
curl -s -X PUT "$BASE_URL/stores" -H 'Content-Type: application/json' \
  -d '{"settings":{"number_of_shards":1,"number_of_replicas":0},"mappings":{"properties":{"name":{"type":"text"},"city":{"type":"keyword"},"location":{"type":"geo_point"},"stock":{"type":"integer"}}}}'
...
' | jq -r '"Loaded: \(.items | length) errors: \(.errors)"' 2>/dev/null

echo ""
echo "=== Magasins dans 300km de Paris ==="
curl -s -X GET "$BASE_URL/stores/_search" -H 'Content-Type: application/json' \
  -d '{"query":{"geo_distance":{"distance":"300km","location":{"lat":48.8566,"lon":2.3522}}},"sort":[{"_geo_distance":{"location":{"lat":48.8566,"lon":2.3522},"order":"asc","unit":"km"}}]}' 
echo ""
echo "=== Bounding box France ==="
curl -s -X GET "$BASE_URL/stores/_search" -H 'Content-Type: application/json' \
  -d '{"query":{"geo_bounding_box":{"location":{"top_left":{"lat":51.0,"lon":-2.0},"bottom_right":{"lat":43.0,"lon":6.0}}}}}' 
echo ""
echo "=== Agrégation geo_distance ==="
curl -s -X GET "$BASE_URL/stores/_search" -H 'Content-Type: application/json' \
  -d '{"size":0,"aggs":{"zones":{"geo_distance":{"field":"location","origin":{"lat":48.8566,"lon":2.3522},"unit":"km","ranges":[{"key":"<100km","to":100},{"key":"100-400km","from":100,"to":400},{"key":">400km","from":400}]},"aggs":{"stock":{"sum":{"field":"stock"}}}}}}' 