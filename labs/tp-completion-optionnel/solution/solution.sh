...
curl -s -X PUT "$BASE_URL/products-suggest" -H 'Content-Type: application/json' \
  -d '{"settings":{"number_of_shards":1,"number_of_replicas":0},"mappings":{"properties":{"name":{"type":"text"},"category":{"type":"keyword"},"price":{"type":"float"},"suggest":{"type":"completion","analyzer":"simple","max_input_length":50}}}}' 
echo ""
...
' | jq -r '"Loaded: \(.items | length)"' 2>/dev/null

echo ""
echo "=== Completion basique : prefix 'smart' ==="
curl -s -X GET "$BASE_URL/products-suggest/_search" -H 'Content-Type: application/json' \
  -d '{"suggest":{"ps":{"prefix":"smart","completion":{"field":"suggest","size":5}}}}' 
echo ""
echo "=== Completion fuzzy : prefix 'smartphne' (faute) ==="
curl -s -X GET "$BASE_URL/products-suggest/_search" -H 'Content-Type: application/json' \
  -d '{"suggest":{"ps":{"prefix":"smartphne","completion":{"field":"suggest","size":5,"fuzzy":{"fuzziness":1,"min_length":4,"prefix_length":2}}}}}' 