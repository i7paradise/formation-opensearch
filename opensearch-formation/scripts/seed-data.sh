...
# =============================================================================
# seed-data.sh — Chargement des données de démonstration pour la formation
...
# Répertoire des données (relatif à la racine du projet)
...
MAX_RETRIES=60  # Nombre maximum de tentatives (60 x 2s = 120s max)
RETRY_COUNT=0

while true; do
  HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" --connect-timeout 3 "${BASE_URL}/_cluster/health" 2>/dev/null || echo "000")
  ...
  if [[ ${RETRY_COUNT} -ge ${MAX_RETRIES} ]]; then
    ...
    log_error "Consultez les logs : docker logs opensearch"
    exit 1
  fi
...
RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" -X PUT "${BASE_URL}/products" \
...
RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" -X PUT "${BASE_URL}/stores" \
  -H "Content-Type: application/json" \
  -d '{
    "settings": {
      "number_of_shards": 1,
...
RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" -X PUT "${BASE_URL}/logs" \
  -H "Content-Type: application/json" \
  -d '{
    "settings": {
      "number_of_shards": 1,
...
if [[ -f "${PRODUCTS_BULK}" ]]; then
  ...
  RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${BASE_URL}/_bulk" \
  ...
  # Données de démonstration intégrées si le fichier est absent
  curl -s -o /dev/null -X POST "${BASE_URL}/_bulk" \
    -H "Content-Type: application/x-ndjson" \
...
if [[ -f "${STORES_FILE}" ]]; then
  ...
  # Lire chaque entrée du tableau JSON et l'indexer individuellement
  STORE_COUNT=0
  # Utilisation de python3 pour parser le JSON si disponible
  if command -v python3 &>/dev/null; then
    python3 - <<'PYEOF'
import json, sys, urllib.request

base_url = "http://localhost:9200"
stores_file = sys.argv[1] if len(sys.argv) > 1 else None

# Chemin du fichier (passé via variable d'environnement ou argument)
import os
stores_path = os.environ.get("STORES_FILE", "")
if not stores_path:
    print("[AVERT] Variable STORES_FILE non définie")
    sys.exit(0)
...
try:
    ...
    for i, store in enumerate(stores):
        doc_id = store.get("id", i + 1)
        data = json.dumps(store).encode("utf-8")
        req = urllib.request.Request(
            f"{base_url}/stores/_doc/{doc_id}",
            data=data,
    ...
    print(f"[OK] {len(stores)} magasins chargés")
except FileNotFoundError:
    print(f"[AVERT] Fichier non trouvé : {stores_path}")
except Exception as e:
    print(f"[AVERT] Erreur lors du chargement des magasins : {e}")
PYEOF
  else
    log_warn "python3 non disponible — chargement magasins ignoré"
    log_warn "Chargement de magasins de démonstration intégrés..."
    curl -s -o /dev/null -X PUT "${BASE_URL}/stores/_doc/1" \
      -H "Content-Type: application/json" \
    ...
    curl -s -o /dev/null -X PUT "${BASE_URL}/stores/_doc/2" \
      -H "Content-Type: application/json" \
    ...
    curl -s -o /dev/null -X PUT "${BASE_URL}/stores/_doc/3" \
      -H "Content-Type: application/json" \
...
if [[ -f "${LOGS_FILE}" ]]; then
  ...
  if command -v python3 &>/dev/null; then
    LOGS_FILE="${LOGS_FILE}" python3 - <<'PYEOF'
import json, sys, urllib.request, os

base_url = "http://localhost:9200"
logs_path = os.environ.get("LOGS_FILE", "")
...
try:
    ...
    for i, log_entry in enumerate(logs):
        doc_id = log_entry.get("id", i + 1)
        data = json.dumps(log_entry).encode("utf-8")
        req = urllib.request.Request(
            f"{base_url}/logs/_doc/{doc_id}",
            data=data,
            headers={"Content-Type": "application/json"},
            method="PUT"
        )
    ...
    print(f"[OK] {len(logs)} entrées de log chargées")
except FileNotFoundError:
    print(f"[AVERT] Fichier non trouvé : {logs_path}")
except Exception as e:
    print(f"[AVERT] Erreur lors du chargement des logs : {e}")
PYEOF
  fi
else
  ...
  # Génération de quelques entrées de log de démonstration
  ...
  for i in 1 2 3 4 5; do
    ...
    IDX=$((i - 1))
    curl -s -o /dev/null -X PUT "${BASE_URL}/logs/_doc/${i}" \
      -H "Content-Type: application/json" \
...
curl -s -o /dev/null -X POST "${BASE_URL}/products,stores,logs/_refresh"
sleep 1
...
# Pause pour laisser OpenSearch finaliser l'indexation
sleep 2

# Récupération des statistiques
CAT_RESPONSE=$(curl -s "${BASE_URL}/_cat/indices?v&h=index,docs.count,store.size,health" 2>/dev/null || echo "")
...
for INDEX in products stores logs; do
  COUNT=$(curl -s "${BASE_URL}/${INDEX}/_count" 2>/dev/null | grep -oE '"count":[0-9]+' | grep -oE '[0-9]+' || echo "0")
...
echo -e "  Exemple de requête  : ${CYAN}${BASE_URL}/products/_search?pretty${NC}"
